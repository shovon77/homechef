    ...
    const tryScroll = () => {
      ...
      if (Platform.OS === 'web' && typeof document !== 'undefined') {
        try {
          ...
          if (el) {
            // Find the nearest scrollable ancestor (RNW ScrollView is usually an overflow:auto div)
            ...
            const offset = NAVBAR_HEIGHT + 8;

            // Prefer the ScrollView's actual scroll container when available.
            const scrollRoot =
              (pageScrollRef.current?.getScrollableNode?.() as HTMLElement | null) ??
              (document.scrollingElement as HTMLElement | null);

            const scrollToIn = (root: HTMLElement) => {
              const top =
            ...
            if (scrollRoot) {
              scrollToIn(scrollRoot);
            } else if (parent) {
              scrollToIn(parent);
            } else {
              const winTop = el.getBoundingClientRect().top + window.scrollY - offset;
              try {
