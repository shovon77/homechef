set -e
mkdir -p .backup lib app/dish

############################################
# 1) Cart helper (localStorage on web)
############################################
[ -f lib/cart.ts ] && cp lib/cart.ts ".backup/cart.ts.$(date +%s)"
cat > lib/cart.ts <<'EOF'
type CartItem = {
  id: number;
  name?: string | null;
...
############################################
# 2) Enhance ChefDetailView: clickable cards + Add to cart
############################################
cp app/chef/ChefDetailView.tsx ".backup/ChefDetailView.tsx.$(date +%s)"

cat > app/chef/ChefDetailView.tsx <<'EOF'
'use client';
...
############################################
# 3) (Optional) Create a route alias /dish/[id].tsx if missing
############################################
if [ ! -f app/dish/[id].tsx ]; then
  mkdir -p app/dish
...
/** Alias route placeholder.
 * If your real dish detail route is elsewhere, update this to re-export or navigate there.
 */
export default function DishAlias() {
  const { id } = useLocalSearchParams();
  return (
    <View style={{ flex:1, alignItems:'center', justifyContent:'center', padding:16 }}>
      <Text>Dish detail placeholder for ID: {String(Array.isArray(id)?id[0]:id)}</Text>
      <Text style={{ marginTop:8, opacity:0.7 }}>Wire this to your real dish detail page if needed.</Text>
    </View>
  );
}
EOF
...
echo "✅ Dish cards now clickable + Add to cart added. If your real dish detail route differs, update the alias at app/dish/[id].tsx."
